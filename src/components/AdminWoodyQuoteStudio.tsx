...
export const AdminWoodyQuoteStudio: React.FC = () => {
  ...
  // Excel File Upload & Data Import state
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-2 max-w-2xl">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-xs font-bold">
                ...
                <span>Admin Generated Only</span>
              </div>
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              Official Woody-Quote commercial quotations are created strictly by Admin review. Quotations are not generated automatically by the system. Configure custom pricing, discounts, and dispatch via WhatsApp, Gmail, or PDF.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            ...
            <button
              onClick={() => {
                ...
                showToast('Template Downloaded', 'Woody-Quote Excel sample template saved to your downloads.');
              }}
              className="bg-white/10 hover:bg-white/20 text-slate-200 border border-white/20 font-bold px-3 py-2.5 rounded-xl text-xs flex items-center gap-1.5 backdrop-blur-md transition-colors cursor-pointer"
              title="Download ready-to-fill Excel template for bulk import"
            >
      ...
      {/* 5. WOODY-QUOTE EXCEL DATA IMPORTER MODAL */}
      {isExcelModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl max-w-5xl w-full max-h-[92vh] flex flex-col overflow-hidden border border-slate-200">
            {/* Modal Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-emerald-900 via-slate-900 to-slate-950 text-white shrink-0">
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => {
                    downloadWoodyQuoteExcelTemplate(zohoSettings);
                    showToast('Template Downloaded', 'Woody-Quote Excel sample template saved.');
                  }}
                  className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-bold border border-white/20 transition-colors cursor-pointer"
                >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              ...
              {!parsedExcelResult ? (
                <div
                  ...
                  className="border-2 border-dashed border-emerald-300 hover:border-emerald-500 bg-emerald-50/50 hover:bg-emerald-50/80 transition-all rounded-3xl p-8 sm:p-12 text-center flex flex-col items-center justify-center cursor-pointer group"
                  onClick={() => excelFileInputRef.current?.click()}
                >
                  <div className="w-16 h-16 rounded-3xl bg-emerald-100 group-hover:scale-110 text-emerald-600 flex items-center justify-center mb-4 transition-transform shadow-md shadow-emerald-500/10">
                    {isParsingExcel ? (
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
                    <button
                      type="button"
                      disabled={isParsingExcel}
                      className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold px-5 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-md shadow-emerald-600/30 transition-all"
                    >
                    ...
                    <button
                      ...
                      className="bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 font-bold px-4 py-2.5 rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-sm"
                    >
                  ...
                  {/* Highlights Grid */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-2xl mt-8 pt-6 border-t border-emerald-200/60 text-left">
                    <div className="bg-white/80 p-3 rounded-2xl border border-emerald-100 shadow-sm">
                      <div className="flex items-center gap-2 text-emerald-700 font-extrabold text-xs">
                        ...
                        <span>Multi-Item Grouping</span>
                      </div>
                      <p className="text-[11px] text-slate-500 mt-1">
                        Rows sharing the same Quote Number are automatically combined into single quotes with multiple line items.
                      </p>
                    </div>
                    ...
                    <div className="bg-white/80 p-3 rounded-2xl border border-emerald-100 shadow-sm">
                      ...
                      <p className="text-[11px] text-slate-500 mt-1">
                        Discounts, subtotals, shipping fees, and grand totals in Kenyan Shillings (KSh) are computed automatically.
                      </p>
                    </div>
                    ...
                    <div className="bg-white/80 p-3 rounded-2xl border border-emerald-100 shadow-sm">
                      ...
                      <p className="text-[11px] text-slate-500 mt-1">
                        Kenyan mobile numbers, email addresses, delivery locations, and dates are preserved cleanly.
                      </p>
                    </div>
                  </div>
                </div>
              ) : (
                /* Parsed Data Preview & Confirmation View */
                <div className="space-y-5">
                  {/* File Metadata & Quick Stats Bar */}
                  <div className="bg-slate-900 text-white p-4 sm:p-5 rounded-2xl flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-sm text-white truncate max-w-xs sm:max-w-md">
                            {excelFileName}
                          </span>
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs text-slate-300 mt-1">
                          <span>Sheets: {parsedExcelResult.detectedSheets.join(', ') || 'Default'}</span>
                          <span>•</span>
                          <span>{parsedExcelResult.totalRows} Raw Rows</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Quotes</div>
                        <div className="text-lg font-black text-emerald-400">{parsedExcelResult.quotes.length}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Line Items</div>
                        <div className="text-lg font-black text-blue-400">{parsedExcelResult.totalItems}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Combined Value</div>
                        <div className="text-lg font-black text-white">
                          KSh {parsedExcelResult.quotes.reduce((sum, q) => sum + q.grandTotal, 0).toLocaleString()}
                        </div>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => excelFileInputRef.current?.click()}
                        className="px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-bold border border-white/20 transition-colors cursor-pointer shrink-0"
                      >
                        Change File
                      </button>
                    </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 mt-0.5">
                            Preserves existing quotations. Quotes with matching Quote # will be updated with new Excel data, and all new quotes will be added.
                          </p>
                        </div>
                      </label>
                      ...
                      <label
                        className={`flex items-start gap-3 p-3.5 rounded-xl border transition-all cursor-pointer ${
                          excelImportMode === 'replace'
                            ? 'bg-red-50/70 border-red-400 ring-2 ring-red-500/20 shadow-sm'
                            : 'bg-white border-slate-200 hover:border-slate-300'
                        }`}
                      >
                        <input
                        ...
                        <div>
                          <div className="text-xs font-extrabold text-slate-900 flex items-center gap-1.5">
                            <span>Replace All Current Quotations</span>
                            <span className="text-[10px] font-bold px-1.5 py-0.5 bg-red-100 text-red-800 rounded">
                              Overwrite
                            </span>
                          </div>
                          <p className="text-[11px] text-slate-500 mt-0.5">
                            Clears the existing database of quotations and replaces it strictly with the {parsedExcelResult.quotes.length} quotes from this Excel file.
                          </p>
                        </div>
                      </label>
                    </div>
                  ...
                  {parsedExcelResult.warnings.length > 0 && (
                    <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 text-amber-900 text-xs">
                      <div className="flex items-center gap-2 font-bold mb-1">
                        <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
                        <span>Spreadsheet Notices ({parsedExcelResult.warnings.length}):</span>
                      </div>
                      <ul className="list-disc list-inside space-y-0.5 text-[11px] text-amber-800 pl-1">
                        {parsedExcelResult.warnings.slice(0, 3).map((w, i) => (
                          <li key={i}>{w}</li>
                        ))}
                        {parsedExcelResult.warnings.length > 3 && (
                          <li>...and {parsedExcelResult.warnings.length - 3} more items handled with sensible defaults.</li>
                        )}
                      </ul>
                    </div>
                  )}
                  ...
                  <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                    <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider">
                          Parsed Quotations Preview ({parsedExcelResult.quotes.length})
                        </h4>
                        <span className="text-[11px] text-slate-500">
                          Click any quote to expand and inspect line items
                        </span>
                      </div>
                    ...
                    <div className="divide-y divide-slate-100 max-h-80 overflow-y-auto">
                      {parsedExcelResult.quotes
                        .filter((q) => {
                          if (!excelSearchFilter.trim()) return true;
                          const f = excelSearchFilter.toLowerCase();
                          return (
                            q.quoteNumber.toLowerCase().includes(f) ||
                            q.customerName.toLowerCase().includes(f) ||
                            q.customerPhone.includes(f) ||
                            (q.companyName && q.companyName.toLowerCase().includes(f))
                          );
                        })
                        .map((q, qIdx) => {
                          const isExpanded = previewExpandedQuoteId === q.id;
                          return (
                            <div key={q.id || qIdx} className="hover:bg-slate-50/80 transition-colors">
                              <div
                                onClick={() => setPreviewExpandedQuoteId(isExpanded ? null : q.id)}
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-xs text-slate-600 mt-0.5 flex items-center gap-2">
                                      ...
                                      {q.companyName && (
                                        <span className="text-[11px] text-slate-500">({q.companyName})</span>
                                      )}
                                      <span className="text-slate-400">•</span>
                                      <span className="text-[11px] text-slate-500">{q.customerPhone}</span>
                                    </div>
                                  </div>
                                </div>
                              ...
                              {isExpanded && (
                                <div className="px-4 pb-4 pt-1 bg-slate-50/90 border-t border-slate-100">
                                  <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                                    Quotation Line Items Breakdown:
                                  </div>
                                  <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                                    <table className="w-full text-left text-xs">
                                      <thead className="bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-wider border-b">
                                        <tr>
                                      ...
                                      <tbody className="divide-y divide-slate-100">
                                        {q.items.map((item, itemI) => (
                                          <tr key={item.id || itemI} className="hover:bg-slate-50">
                                            <td className="py-2 px-3">
                                              ...
                                              {(item.selectedSize || item.selectedFinish) && (
                                                <div className="text-[10px] text-blue-600 font-semibold mt-0.5">
                                                  {[item.selectedSize, item.selectedFinish].filter(Boolean).join(' • ')}
                                                </div>
                                              )}
                                            </td>
                                  ...
                                  <div className="flex flex-wrap items-center justify-between text-xs text-slate-600 mt-2.5 px-1">
                                    <div>
                                      <span className="font-semibold text-slate-700">Delivery:</span> {q.deliveryType} ({q.deliveryLocation})
                                    </div>
                                    <div className="flex items-center gap-3">
                                      <span>Subtotal: KSh {q.subtotal.toLocaleString()}</span>
                                      {q.shippingCost > 0 && <span>Shipping: KSh {q.shippingCost.toLocaleString()}</span>}
                                      <span className="font-black text-slate-900">Grand Total: KSh {q.grandTotal.toLocaleString()}</span>
                                    </div>
                                  </div>
                                </div>
                              )}
            ...
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-3 shrink-0">
              <div className="text-xs text-slate-500">
                {!parsedExcelResult ? (
                  <span>Need an Excel format to copy? Use the <strong>Download Sample Template</strong> button.</span>
                ) : (
                  <span>Ready to import <strong>{parsedExcelResult.quotes.length}</strong> quotations with <strong>{parsedExcelResult.totalItems}</strong> items.</span>
                )}
              </div>
