...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const importZohoQuotations = (quotesList: ZohoQuotation[], mode: 'append' | 'replace' = 'append'): { added: number; updated: number; total: number } => {
    if (!quotesList || quotesList.length === 0) {
      showToast('Import Notice', 'No quotation records were found in the uploaded file.', 'info');
      return { added: 0, updated: 0, total: zohoQuotations.length };
    }

    if (mode === 'replace') {
      setZohoQuotations(quotesList);
      safeSetLocalStorage('pixelprint_zoho_quotations', quotesList);
      showToast('Excel Data Imported!', `Replaced quotation database with ${quotesList.length} quotes from Excel.`);
      return { added: quotesList.length, updated: 0, total: quotesList.length };
    }
    ...
    setZohoQuotations((prev) => {
      const existingMap = new Map<string, ZohoQuotation>();
      // Key by quoteNumber or id
      prev.forEach((q) => {
        if (q.quoteNumber) existingMap.set(q.quoteNumber.trim().toUpperCase(), q);
        existingMap.set(q.id, q);
      });

      const updatedList = [...prev];

      quotesList.forEach((incoming) => {
        const key = (incoming.quoteNumber || '').trim().toUpperCase();
        const existingIdx = key ? updatedList.findIndex((q) => (q.quoteNumber || '').trim().toUpperCase() === key) : -1;
      ...
      safeSetLocalStorage('pixelprint_zoho_quotations', updatedList);
      return updatedList;
    });

    showToast('Excel Import Complete!', `Successfully processed ${quotesList.length} quotes (${addedCount} new added, ${updatedCount} updated).`);
    return { added: addedCount, updated: updatedCount, total: zohoQuotations.length + addedCount };
  };
